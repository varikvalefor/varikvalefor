...
\paragraph{English}
VARIK does-not find that necessary is that VARIK uses (for that VARIK explains \F{jbobau-la-varik}) English.  VARIK suggests that for all prenu $A$, if $A$ uses English, then if $A$ desires that $A$ understands about the proof, then if $A$ does-not understands about the Agda definition, then $A$ reads the English summary.
